...
vector<double> JMT(vector<double> start, vector<double> end, double T)
{
    ...
    */
    ...
    // Assign first 3 coefficients
    double a_0 = start[0];
    double a_1 = start[1];
    double a_2 = start[2]/2;

    // Create powers of time variable
    double t_2 = pow(T,2);
    double t_3 = pow(T,3);
    double t_4 = pow(T,4);
    double t_5 = pow(T,5);

    // Create matrix that stores time constants
    MatrixXd time_constants(3, 3);
    time_constants << t_3, t_4, t_5,
    				3*t_2, 4*t_3, 5*t_4,
    				6*T, 12*t_2, 20*t_3;

    // Create vector that stores s constants
    VectorXd s_constants(3);
    s_constants << end[0] - (start[0] + start[1]*T + start[2]*t_2/2),
    				end[1] - (start[1] + start[2]*T),
    				end[2] - start[2];

   	// Create matrix to store last 3 coefficients
    VectorXd coefficients(3);

    //Calculate coefficients
    coefficients = time_constants.inverse() * s_constants;

    return {a_0,a_1,a_2,coefficients[0],coefficients[1],coefficients[2]};
}
